...
export default function RecuperarPasswordPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await fetch("/api/auth/request-password-reset", {
        ...
        credentials: "include",
        body: JSON.stringify({ email: email.trim().toLowerCase(), redirectTo: `${window.location.origin}/restablecer-password` }),
      });
    } finally {
      // Always return the same result to avoid revealing registered accounts.
      setSent(true);
      setLoading(false);
    }
  }
  ...
  return <main className="grid min-h-screen place-items-center bg-slate-950 px-4 text-white">
    <section className="w-full max-w-md rounded-3xl border border-slate-800 bg-slate-900 p-6 shadow-2xl">
      ...
      {sent ? <div className="mt-5 space-y-4"><p className="rounded-2xl bg-emerald-950/50 p-4 text-sm text-emerald-200">Si el correo está registrado, vas a recibir un enlace válido durante una hora.</p><Link href="/login" className="block text-center text-sm font-bold text-cyan-400">Volver al ingreso</Link></div> : <form onSubmit={submit} className="mt-5 space-y-4">
        <p className="text-sm text-slate-400">Ingresá el correo asociado a tu usuario administrativo.</p>
