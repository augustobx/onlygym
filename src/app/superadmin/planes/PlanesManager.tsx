...
function PlanEditor({ plan }: { plan: any }) {
  ...
  return <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-5">
    ...
    <Field label="Descripción"><textarea value={description} onChange={e=>setDescription(e.target.value)} rows={3} className="input resize-none" /></Field>
    <Field label="Precio mensual"><input type="number" value={price} onChange={e=>setPrice(Number(e.target.value))} className="input" /></Field>
...
function CreatePlan(){
  ...
  return <section className="bg-slate-900 border border-slate-800 rounded-2xl p-6"><h2 className="font-bold text-white flex items-center gap-2 mb-4"><Plus className="w-4 h-4 text-emerald-400"/>Crear nuevo plan</h2><form action={submit} className="grid md:grid-cols-4 gap-3"><input name="codigo" required placeholder="Código" className="input"/><input name="nombre" required placeholder="Nombre" className="input"/><input name="precioMensual" required type="number" min="0" placeholder="Precio" className="input"/><input name="descripcion" placeholder="Descripción" className="input"/><input name="limiteUsuarios" type="number" min="1" defaultValue="5" placeholder="Usuarios" className="input"/><input name="limiteSucursales" type="number" min="1" defaultValue="1" placeholder="Sedes" className="input"/><input name="limiteSocios" type="number" min="0" placeholder="Socios (0 ilimitado)" className="input"/><button disabled={loading} className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 text-emerald-300 py-3 font-bold">{loading?"Creando...":"Crear plan"}</button></form>{error&&<p className="text-xs text-red-400 mt-3">{error}</p>}</section>
}
