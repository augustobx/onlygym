...
export async function GET(req: Request) {
  ...
  if (!tenantSlug) return NextResponse.json({ error: "Tenant inválido" }, { status: 404 });
  ...
  try {
    ...
    const branch = await prisma.sucursal.findFirst({
      ...
      include: { horarios: { where: { activo: true }, take: 1 } },
    });
    if (!branch) return NextResponse.json({ error: "Sucursal no encontrada para este tenant" }, { status: 404 });
    ...
    const averageDuration = completed.length
      ? Math.round(completed.reduce((sum, row) => sum + (row.duracionMinutos || 0), 0) / completed.length)
      : 0;

    return NextResponse.json({
      sucursalId: branch.id,
      personasAdentro: peopleInside,
      capacidadMaxima: capacity,
      porcentaje: percentage,
      nivel: percentage >= 90 ? "alto" : percentage >= 60 ? "medio" : "bajo",
      nivelTexto: percentage >= 90 ? "Casi completo" : percentage >= 60 ? "Concurrido" : "Disponible",
      duracionPromedio: averageDuration,
