...
export default function PortalLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    } else if ("suspended" in result && result.suspended) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8 text-slate-100 font-sans selection:bg-cyan-500 selection:text-white">
      <div className="sm:mx-auto sm:w-full sm:max-w-md space-y-3 text-center">
        ...
        <div>
          <h2 className="text-2xl sm:text-3xl font-black text-white tracking-tight">Portal del Socio</h2>
          <p className="text-xs text-slate-400 font-medium mt-0.5">Accede a tu carnet QR digital, estado de cuenta y asistencias</p>
        </div>
      </div>
      ...
      <div className="mt-6 sm:mx-auto sm:w-full sm:max-w-md space-y-4">
        <div className="bg-slate-900/90 backdrop-blur-md py-6 px-6 sm:px-8 rounded-2xl border border-slate-800 shadow-2xl space-y-4">
          <form className="space-y-4" method="post" onSubmit={handleSubmit}>
            ...
            <div>
              <label className="block text-[11px] font-bold text-slate-300 uppercase tracking-wider mb-1.5">DNI o Usuario</label>
              <div className="relative rounded-lg shadow-xs">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><User className="h-4 w-4 text-cyan-400" /></div>
                <input type="text" required value={usuario} onChange={(e) => setUsuario(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded-xl pl-9 pr-3 py-2.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20 font-mono font-bold" placeholder="Tu número de documento" autoComplete="username" autoFocus />
              </div>
            </div>

            <div>
              <label className="block text-[11px] font-bold text-slate-300 uppercase tracking-wider mb-1.5">Contraseña</label>
              <div className="relative rounded-lg shadow-xs">
                ...
                <input type="password" required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded-xl pl-9 pr-3 py-2.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20 font-medium" placeholder="••••••••" autoComplete="current-password" />
              </div>
              <p className="text-[10px] text-slate-500 mt-1.5 font-medium">En tu primer ingreso usá la <strong className="text-cyan-400">contraseña temporal entregada por el gimnasio</strong>.</p>
            </div>
