    ...
    <div className="mx-auto max-w-6xl space-y-5 font-sans">
      ...
      <header className="flex flex-col gap-3 rounded-xl border border-slate-200/90 bg-white p-5 shadow-2xs sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <h1 className="mt-1 flex items-center gap-2 text-xl font-bold tracking-tight text-slate-900"><CreditCard className="h-5 w-5 text-cyan-600" />Cobrar membresía</h1>
        ...
        <div className="flex items-center gap-2 rounded-lg border border-cyan-200 bg-cyan-50 px-3 py-2 text-xs font-bold text-cyan-950"><MapPin className="h-3.5 w-3.5 text-cyan-600" />{sucursalNombre}</div>
      </header>
      ...
      <div className="grid items-start gap-5 lg:grid-cols-12">
        <section className="space-y-5 rounded-xl border border-slate-200/90 bg-white p-5 shadow-2xs lg:col-span-7">
          <div className="space-y-2">
            <div className="flex items-center justify-between"><label className="text-xs font-black uppercase tracking-wider text-slate-800">1 · Socio</label>{selectedCliente && <button onClick={clearCliente} className="text-xs font-bold text-cyan-700 hover:underline">Cambiar socio</button>}</div>
            {selectedCliente ? (
          ...
          <div className="space-y-2"><label className="text-xs font-black uppercase tracking-wider text-slate-800">2 · Plan</label><div className="grid gap-2 sm:grid-cols-2">{membresias.map((membresia) => { const active = selectedMembresia?.id === membresia.id; return <button key={membresia.id} type="button" onClick={() => setSelectedMembresia(membresia)} className={`rounded-xl border p-3 text-left transition ${active ? "border-cyan-500 bg-cyan-50 ring-2 ring-cyan-500/20" : "border-slate-200 bg-white hover:border-slate-300"}`}><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-black text-slate-950">{membresia.nombre}</p><p className="text-[11px] font-semibold text-slate-500">{membresia.diasDuracion} días</p></div><strong className="font-mono text-sm text-slate-950">{formatMoney(membresia.precio)}</strong></div></button>; })}</div></div>

          <div className="space-y-2"><label className="text-xs font-black uppercase tracking-wider text-slate-800">3 · Medio de pago</label><div className="grid grid-cols-3 gap-2">{([{ id: "efectivo", label: "Efectivo", icon: Banknote }, { id: "tarjeta", label: "Tarjeta", icon: WalletCards }, { id: "transferencia", label: "Transferencia", icon: CreditCard }] as const).map(({ id, label, icon: Icon }) => <button key={id} type="button" onClick={() => setMetodoPago(id)} className={`flex min-h-16 flex-col items-center justify-center gap-1 rounded-xl border text-xs font-bold ${metodoPago === id ? "border-slate-950 bg-slate-950 text-white" : "border-slate-200 bg-white text-slate-700 hover:bg-slate-50"}`}><Icon className="h-4 w-4" />{label}</button>)}</div></div>

          <div className="space-y-2"><label className="text-xs font-black uppercase tracking-wider text-slate-800">Nota <span className="font-medium text-slate-400">(opcional)</span></label><input value={notas} onChange={(event) => setNotas(event.target.value)} placeholder="Ej: transferencia recibida, observación…" className="h-10 w-full rounded-xl border border-slate-300 px-3 text-sm outline-none focus:border-cyan-500" /></div>
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-4"><div className="flex items-center justify-between"><span className="text-sm font-bold text-slate-700">Total</span><strong className="font-mono text-xl text-slate-950">{selectedMembresia ? formatMoney(selectedMembresia.precio) : "$0,00"}</strong></div><p className="mt-1 text-[10px] font-medium text-slate-500">El importe se toma del plan configurado en el servidor.</p></div>
          <button onClick={handlePago} disabled={!selectedCliente || !selectedMembresia || loading} className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-cyan-600 to-blue-600 text-sm font-black text-white shadow-sm disabled:cursor-not-allowed disabled:opacity-40"><CreditCard className="h-4 w-4" />{loading ? "Registrando cobro…" : selectedMembresia ? `Confirmar ${formatMoney(selectedMembresia.precio)}` : "Seleccioná socio y plan"}</button>
        </section>

        <aside className="overflow-hidden rounded-xl border border-slate-200/90 bg-white shadow-2xs lg:col-span-5"><div className="border-b border-slate-100 px-4 py-3"><h2 className="text-xs font-black uppercase tracking-wider text-slate-900">Cobros de hoy</h2><p className="mt-0.5 text-[11px] font-medium text-slate-500">{pagosHoy.length} operaciones en {sucursalNombre}</p></div><div className="max-h-[620px] divide-y divide-slate-100 overflow-y-auto">{pagosHoy.length === 0 ? <p className="p-8 text-center text-xs font-medium text-slate-500">Todavía no hay cobros registrados hoy.</p> : pagosHoy.map((pago: any) => <div key={pago.id} className="flex items-center justify-between gap-3 px-4 py-3 hover:bg-slate-50"><div className="min-w-0"><Link href={`/dashboard/clientes/${pago.clienteId}`} className="block truncate text-xs font-black text-slate-950 hover:text-cyan-700">{pago.cliente?.nombre} {pago.cliente?.apellido}</Link><p className="mt-0.5 text-[10px] font-medium text-slate-500">{pago.membresia?.nombre} · {new Date(pago.fechaPago).toLocaleTimeString("es-AR", { hour: "2-digit", minute: "2-digit" })} · {pago.metodoPago || "efectivo"}</p></div><span className="shrink-0 font-mono text-xs font-black text-slate-950">{formatMoney(pago.monto)}</span></div>)}</div></aside>
      </div>
    </div>
  );
}
