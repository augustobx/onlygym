...
export default function MovimientosPage() {
  ...
  return (
    <div className="space-y-5 font-sans max-w-7xl mx-auto">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-600 font-medium mt-0.5">
              Sede: <strong className="text-slate-900">{sucursalNombre}</strong> — Auditoría de ventas en cantina y cobros de cuotas.
            </p>
          </div>
        </div>
      ...
      <form onSubmit={handleFiltrar} className="bg-white p-3.5 rounded-xl border border-slate-200/90 shadow-2xs flex flex-wrap items-center gap-3 text-xs">
        <div className="flex items-center gap-1.5 text-slate-800">
          ...
          <input
            ...
            className="bg-white border border-slate-300 rounded-lg px-2.5 py-1 text-slate-900 font-mono text-xs focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 focus:outline-none"
          />
        </div>
        ...
        {activeTab === "pos" && (
          <div className="flex items-center gap-1.5 text-slate-800">
            ...
            <select
              ...
              className="bg-white border border-slate-300 text-slate-900 rounded-lg px-2.5 py-1 text-xs font-semibold focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 focus:outline-none"
            >
        ...
        <button
          type="submit"
          disabled={loading}
          className="px-3.5 py-1.5 bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-700 hover:to-blue-700 text-white rounded-lg text-xs font-semibold shadow-xs transition"
        >
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
            <div className="bg-white p-3.5 rounded-xl border border-slate-200/90 shadow-2xs space-y-1">
              <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500 block">Total Recaudado</span>
              <p className="text-base font-bold font-mono text-slate-900 tabular-nums">{formatMoney(resumenPOS.totalGeneral)}</p>
              <span className="text-[10px] text-slate-600 font-medium block">{resumenPOS.totalVentas} ventas</span>
            </div>

            <div className="bg-white p-3.5 rounded-xl border border-slate-200/90 shadow-2xs space-y-1">
              <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500 block">Efectivo</span>
              <p className="text-base font-bold font-mono text-emerald-700 tabular-nums">{formatMoney(resumenPOS.totalEfectivo)}</p>
              <span className="text-[10px] text-slate-600 font-medium block">Caja física</span>
            </div>

            <div className="bg-white p-3.5 rounded-xl border border-slate-200/90 shadow-2xs space-y-1">
              <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500 block">Tarjetas</span>
              <p className="text-base font-bold font-mono text-cyan-700 tabular-nums">{formatMoney(resumenPOS.totalTarjeta)}</p>
              <span className="text-[10px] text-slate-600 font-medium block">Débito / Crédito</span>
            </div>

            <div className="bg-white p-3.5 rounded-xl border border-slate-200/90 shadow-2xs space-y-1">
              <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500 block">Transferencias</span>
              <p className="text-base font-bold font-mono text-blue-700 tabular-nums">{formatMoney(resumenPOS.totalTransferencia)}</p>
              <span className="text-[10px] text-slate-600 font-medium block">Bancario / MP</span>
            </div>

            <div className="bg-white p-3.5 rounded-xl border border-slate-200/90 shadow-2xs space-y-1">
              <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500 block">A Cuenta (Fiado)</span>
              <p className="text-base font-bold font-mono text-amber-700 tabular-nums">{formatMoney(resumenPOS.totalCuentaCorriente)}</p>
              <span className="text-[10px] text-slate-600 font-medium block">Saldo deudor</span>
            </div>
          </div>
                  ...
                  {ventasPOS.length === 0 ? (
                    ...
                    ventasPOS.map(v => (
                      <tr key={v.id} className="hover:bg-slate-50/70 transition">
                        ...
                        <td className="px-4 py-2.5 font-mono text-[11px] text-slate-600">{new Date(v.fecha).toLocaleString("es-AR")}</td>
                        <td className="px-4 py-2.5 font-bold text-slate-900">
                          {v.cliente ? `${v.cliente.nombre} ${v.cliente.apellido}` : <span className="text-slate-500 italic">Consumidor Final</span>}
                        </td>
                        <td className="px-4 py-2.5 text-center font-mono font-semibold">{v.detalles?.length || 0}</td>
                        ...
                        <td className="px-4 py-2.5 text-right">
                          <button
                            ...
                            className="inline-flex items-center gap-1 px-2.5 py-1 bg-white hover:bg-slate-50 text-slate-800 border border-slate-300 rounded-md text-xs font-semibold shadow-2xs transition"
                          >
      ...
      {activeTab === "membresias" && (
        <div className="space-y-4">
          <div className="bg-white p-4 rounded-xl border border-slate-200/90 shadow-2xs flex items-center justify-between">
            <span className="text-xs font-bold text-slate-800 uppercase tracking-wider">Total Cobrado en Membresías Hoy</span>
            <span className="text-xl font-bold font-mono text-cyan-700 tabular-nums">{formatMoney(totalMembresias)}</span>
          </div>
                  ...
                  {movimientosMembresias.length === 0 ? (
                    ...
                    movimientosMembresias.map(m => (
                      <tr key={m.id} className="hover:bg-slate-50/70 transition">
                        <td className="px-4 py-2.5 font-mono text-[11px] text-slate-600">{new Date(m.fechaPago).toLocaleTimeString("es-AR")}</td>
                        <td className="px-4 py-2.5 font-bold text-slate-900">{m.cliente?.nombre} {m.cliente?.apellido}</td>
                        <td className="px-4 py-2.5 text-slate-700 font-medium">{m.membresia?.nombre}</td>
                        <td className="px-4 py-2.5 font-mono text-[11px] text-slate-600">{new Date(m.fechaVencimiento).toLocaleDateString("es-AR")}</td>
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-2xs flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-sm w-full overflow-hidden border border-slate-200 animate-in fade-in zoom-in-95 duration-100">
            <div className="bg-slate-900 p-4 text-white text-center relative">
              ...
              <Receipt className="h-5 w-5 mx-auto mb-1 text-cyan-400" />
