    ...
    <div className="space-y-5 font-sans max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
        ...
        <div className="lg:col-span-4 bg-white rounded-xl border border-slate-200/90 shadow-2xs flex flex-col h-[600px] overflow-hidden">
          <div className="p-3 border-b border-slate-100 space-y-2 bg-slate-50/70">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-1.5 bg-white border border-slate-200 text-slate-900 rounded-lg text-xs font-medium focus:ring-1 focus:ring-indigo-500 focus:outline-none placeholder-slate-400"
              />
            </div>
            <select
              value={filtro}
              onChange={e => setFiltro(e.target.value)}
              className="w-full bg-white border border-slate-200 text-slate-900 rounded-lg px-2.5 py-1.5 text-xs font-medium focus:ring-1 focus:ring-indigo-500 focus:outline-none"
            >
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-slate-100 text-xs">
            {cuentasFiltradas.length === 0 ? (
              <p className="p-8 text-center text-slate-400 font-medium">No se encontraron cuentas.</p>
            ) : (
                ...
                >
                  <div className="flex justify-between items-start">
                    <div className="truncate">
                      <p className="font-semibold text-slate-900 truncate">{c.cliente.nombre} {c.cliente.apellido}</p>
                      <p className="text-[11px] text-slate-500 font-mono">DNI: {c.cliente.documento}</p>
                    </div>
                    <div className="text-right flex-shrink-0">
                      <p className={`font-bold font-mono tabular-nums ${c.saldo > 0 ? "text-rose-600" : "text-slate-700"}`}>
                        {formatMoney(c.saldo)}
                      </p>
                      {c.limiteCredito > 0 && (
                        <p className="text-[10px] text-slate-400 font-mono">Lím: {formatMoney(c.limiteCredito)}</p>
                      )}
                    </div>
                  </div>
                </button>
        ...
        <div className="lg:col-span-8 space-y-5">
          {!cuentaActiva ? (
            <div className="bg-white rounded-xl border border-slate-200/90 shadow-2xs h-80 flex flex-col items-center justify-center p-6 text-center text-slate-400 space-y-1.5">
              <User className="w-8 h-8 text-slate-300" />
              <p className="text-xs font-semibold text-slate-700">Selecciona un socio de la lista izquierda</p>
              <p className="text-[11px] text-slate-400">Podrás consultar su libro de movimientos, asentar pagos o registrar cargos.</p>
            </div>
          ) : (
            <>
              {/* Tarjeta Resumen */}
              <div className="bg-white rounded-xl border border-slate-200/90 shadow-2xs p-5 space-y-4">
                <div className="flex flex-col sm:flex-row justify-between items-start gap-4 pb-3 border-b border-slate-100">
                  <div>
                    ...
                    <span className="text-xs text-slate-500 font-mono">DNI: {cuentaActiva.cliente.documento}</span>
                    <Link
                      href={`/dashboard/clientes/${cuentaActiva.clienteId}`}
                      className="text-indigo-600 text-xs font-medium hover:underline mt-1 block"
                    >
                      Ver Ficha 360 del socio →
                    </Link>
                  </div>
                  
                  <div className="text-left sm:text-right p-3 bg-slate-50/80 rounded-lg border border-slate-200 min-w-[180px]">
                    <span className="text-[10px] font-semibold uppercase tracking-wider text-slate-500 block">Saldo Deudor</span>
                    ...
                    <span className="text-[11px] text-slate-500 font-mono block">Límite: {formatMoney(cuentaActiva.limiteCredito)}</span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                  <button
                    ...
                    className="inline-flex items-center justify-center gap-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 py-2 px-3 rounded-lg font-semibold text-xs transition"
                  >
                  ...
                  <button
                    ...
                    className="inline-flex items-center justify-center gap-1.5 bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 py-2 px-3 rounded-lg font-semibold text-xs transition"
                  >
                  ...
                  <button
                    ...
                    className="inline-flex items-center justify-center gap-1.5 bg-slate-50 hover:bg-slate-100 text-slate-700 border border-slate-200 py-2 px-3 rounded-lg font-medium text-xs transition"
                  >
                    ...
                    <tbody className="divide-y divide-slate-100 text-slate-700">
                      {movimientos.map(m => (
                        <tr key={m.id} className="hover:bg-slate-50/70 transition">
                          <td className="px-3.5 py-2 text-slate-500 font-mono text-[11px]">{formatDateTime(m.fecha)}</td>
                          <td className="px-3.5 py-2 font-medium text-slate-900">{m.concepto || "—"}</td>
                          <td className="px-3.5 py-2 text-slate-500">{m.usuario}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-2xs p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-5 space-y-4 border border-slate-200 animate-in fade-in zoom-in-95 duration-100">
            ...
            <div className="space-y-3 text-xs">
              ...
              {showModal !== "limite" && (
                <div>
                  <label className="block font-medium text-slate-700 mb-1">Concepto / Detalle (Opcional)</label>
                  <input
                    type="text"
                    value={concepto}
                    onChange={e => setConcepto(e.target.value)}
                    placeholder="Ej. Pago en efectivo por mostrador..."
                    className="w-full bg-white border border-slate-300 text-slate-900 rounded-lg px-3 py-2 text-xs focus:ring-1 focus:ring-indigo-500 focus:outline-none"
                  />
                </div>
              )}
            </div>
            ...
            <div className="flex gap-2 pt-2 border-t border-slate-100">
              ...
              <button
                ...
                className={`flex-1 text-white rounded-lg py-2 text-xs font-semibold shadow-2xs transition ${
                  showModal === "pago" 
