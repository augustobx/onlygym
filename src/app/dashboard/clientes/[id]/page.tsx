...
type TemporaryCredential = { password: string; message: string; whatsappUrl: string | null } | null;
...
export default function ClienteDetailPage() {
  ...
  const canFinance = role === "OWNER" || role === "ADMIN" || role === "RECEPCION";
  const canEdit = canFinance;
  const canResetPassword = role === "OWNER" || role === "ADMIN";
  const lastPayment = canFinance ? cliente?.pagos?.[0] || null : null;
  const membership = membershipSnapshot(lastPayment?.fechaVencimiento);
  const account = canFinance ? cliente?.cuentaCorriente || null : null;
  ...
  const reminderUrl = useMemo(() => {
    if (!canFinance || !cliente?.telefono) return null;
    const reminder = buildMembershipReminderMessage({
      ...
      expired: membership.state === "expired",
    });
    return buildWhatsAppUrl(cliente.telefono, reminder);
  }, [branchName, canFinance, cliente, lastPayment, membership.state]);
  ...
  if (loading || role === null) return <div className="py-24 text-center text-sm font-bold text-slate-500">Cargando ficha del socio…</div>;
  ...
  const tabs: Array<{ key: Tab; label: string; icon: typeof UserRound }> = isTrainer
    ? [
  ...
  const membershipLabel = membership.state === "none"
    ? "Sin membresía"
      ...
      <header className="rounded-2xl border border-slate-200 bg-white p-5 shadow-2xs">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex min-w-0 items-center gap-3">
            <Link href={isTrainer ? "/dashboard/entrenador" : "/dashboard/clientes"} className="grid h-9 w-9 shrink-0 place-items-center rounded-xl border border-slate-200 bg-slate-50 text-slate-600 hover:bg-slate-100" title="Volver"><ArrowLeft className="h-4 w-4" /></Link>
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <h1 className="truncate text-xl font-black text-slate-950">{cliente.nombre} {cliente.apellido}</h1>
                {canFinance && <span className={`rounded-full border px-2 py-0.5 text-[10px] font-black ${membership.active ? membership.state === "expiring" ? "border-amber-300 bg-amber-50 text-amber-900" : "border-emerald-300 bg-emerald-50 text-emerald-800" : "border-rose-300 bg-rose-50 text-rose-800"}`}>{membershipLabel}</span>}
                {cliente.estado !== "activo" && <span className="rounded-full border border-slate-300 bg-slate-100 px-2 py-0.5 text-[10px] font-black text-slate-600">SOCIO INACTIVO</span>}
              </div>
          ...
          <div className="flex flex-wrap gap-2">
            {isTrainer ? (
              <>
                <Link href={memberWorkspaceHref("training", clienteId)} className="inline-flex items-center gap-1.5 rounded-xl bg-slate-950 px-3.5 py-2 text-xs font-black text-white"><Dumbbell className="h-3.5 w-3.5" />Planificar entrenamiento</Link>
                <Link href={memberWorkspaceHref("progress", clienteId)} className="inline-flex items-center gap-1.5 rounded-xl border border-slate-300 bg-white px-3 py-2 text-xs font-bold text-slate-800"><TrendingUp className="h-3.5 w-3.5" />Registrar progreso</Link>
              </>
            ) : (
              <>
                ...
                {reminderUrl && <a href={reminderUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 rounded-xl border border-emerald-300 bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-800 hover:bg-emerald-100"><MessageCircle className="h-3.5 w-3.5" />Recordar por WhatsApp</a>}
              </>
            )}
          </div>
        </div>
          ...
          <section className="space-y-4 lg:col-span-8">
            {isTrainer ? (
              <div className="grid gap-3 sm:grid-cols-3">
                ...
                <SummaryCard label="Sedes habilitadas" value={String(cliente.sucursales?.length || 0)} detail={cliente.sucursales?.map((branch: any) => branch.nombre).join(" · ") || "Sin sedes asignadas"} />
                <SummaryCard label="Estado" value={cliente.estado === "activo" ? "Activo" : "Inactivo"} detail="Estado operativo del socio" tone={cliente.estado === "activo" ? "good" : "bad"} />
              </div>
            ) : (
              <div className="grid gap-3 sm:grid-cols-3">
                <SummaryCard label="Membresía" value={lastPayment?.membresia?.nombre || "Sin plan"} detail={lastPayment?.fechaVencimiento ? `Vence ${date(lastPayment.fechaVencimiento)}` : "Todavía no tiene un plan registrado"} tone={membership.active ? "good" : "bad"} />
                <SummaryCard label="Cuenta corriente" value={money(balance)} detail={creditLimit > 0 ? `Disponible ${money(availableCredit)}` : "Sin crédito habilitado"} tone={balance > 0 ? "warn" : "good"} />
                <SummaryCard label="Último ingreso" value={lastAccess ? date(lastAccess.fechaHora) : "Sin ingresos"} detail={lastAccess ? dateTime(lastAccess.fechaHora) : "No hay actividad registrada"} />
              </div>
            )}

            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-2xs">
              <div><p className="text-xs font-black uppercase tracking-wider text-slate-500">Acciones recomendadas</p><h2 className="mt-1 text-lg font-black text-slate-950">Qué podés hacer con este socio</h2></div>
              <div className="mt-4 grid gap-2 sm:grid-cols-2">
                {isTrainer ? (
                  <>
                    <ActionLink href={memberWorkspaceHref("training", clienteId)} icon={Dumbbell} title="Planificar entrenamiento" description="Asignar o ajustar el trabajo del socio." />
                    <ActionLink href={memberWorkspaceHref("progress", clienteId)} icon={TrendingUp} title="Registrar progreso" description="Mediciones, evolución y fotos privadas." />
                    <button onClick={() => setTab("actividad")} className="rounded-xl border border-slate-200 bg-white p-3 text-left hover:border-cyan-300 hover:bg-cyan-50"><span className="flex items-center gap-2 text-sm font-black"><Activity className="h-4 w-4 text-cyan-700" />Revisar actividad</span><span className="mt-1 block text-xs font-medium text-slate-500">Ingresos recientes para contextualizar el seguimiento.</span></button>
                  </>
                ) : (
                  <>
